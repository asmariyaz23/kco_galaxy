...
parser.add_argument('--lib_summary', type=str)
...
lib_file_path = os.path.join(out_path,args.lib_summary)
f=open(lib_file_path, 'w')
f.write('hi there\n')
f.close() 
...
lib_summary = pandas.concat([pandas.read_table(x+'/SMRTanalysis/summary.txt') \
										   for x in cells])
lib_summary_path = os.path.join(out_path,'lib_summary.txt')
...
lib_cuffcmp = out_dir+'/lib_cuffcmp'
os.makedirs(lib_cuffcmp)
cell_gtfs = ' '.join([x+'/SMRTanalysis/align/final.gtf' for x in cells])
...
plot_and_save(xvals, yvals, 'Cumulative unique loci', '#Cells', '#Loci',\
			out_path+'/loci_rarefaction.png')

## Plot cumulative junctions
juncs = []
cols = ['Chr','Start','End','Strand']
for i,cell in enumerate(cells):
	df = pandas.read_table(cell+'/SMRTanalysis/align/junctions.bed', names=\
		cols)
	df['Cell'] = i
	juncs.append(df)
...
plot_and_save(xvals, yvals, 'Cumulative unique junctions', '#Cells', '#Juncs',\
			out_path+'/junctions_rarefaction.png')
